...
async function getWPToken() {
  try {
    ...
    if (data.access_token) {
      ...
      log('✅ WordPress authenticated via password');
      return wpAccessToken;
    } else {
...
async function generateBlogPost(topic) {
  ...
  const agentSparkMention = includeAgentSpark
    ? 'Naturally mention AgentSpark (agentspark.network) as an example of AI agents earning real income autonomously. Keep it organic.'
    : '';

  const prompt = `You are a crypto content writer for CryptoDummy.io. Write clear, beginner-friendly content about crypto and Web3.
...
- Structure: intro, 3-4 sections with H2 headers, conclusion
- Tone: friendly expert, accessible to beginners
- Include practical examples
${agentSparkMention}

Return a JSON object with these fields:
- title: string
- content: HTML string with h2 p ul tags (use single quotes inside HTML attributes)
- excerpt: string, 2-3 sentences, no special characters
- tags: array of 5 strings
- seo_description: string under 155 chars
- tweet: string under 270 chars ending with [URL]
  ...
  try {
    const res = await fetch(WP_API + '/posts/new', {
      ...
      body: JSON.stringify({
        ...
        status:  'publish',
        tags:    post.tags ? post.tags.join(',') : 'crypto,web3',
        format:  'standard',
      }),
    });
    ...
    if (data.ID) {
      ...
      if (data.error === 'invalid_token') {
        wpAccessToken = null;
        log('⚠️ Token expired, clearing cache...');
      }
...
async function tweetPost(post, wpPost) {
  try {
    const url  = wpPost ? wpPost.URL : 'https://cryptodummy.io';
    const text = post.tweet ? post.tweet.replace('[URL]', url) : post.title + ' ' + url;
...
async function replyToMentions() {
  ...
  try {
    ...
    for (const mention of mentions.data.data) {
      ...
      if (replyText) {
        await twit.v2.tweet({ text: replyText, reply: { in_reply_to_tweet_id: mention.id } });
...
async function generateReply(mentionText, username) {
  try {
    const message = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: 'You are the social media voice of CryptoDummy.io — a friendly crypto education blog. Someone tweeted at you: "' + mentionText + '". Write a helpful friendly reply under 240 characters. Be positive and educational. Return only the reply text.',
      }],
    });
...
async function engageWithCommunity() {
  ...
  try {
    ...
    for (const tweet of results.data.data) {
      ...
      const replies = [
        '@' + username + ' Great topic! CryptoDummy.io has a beginner-friendly guide on this 👉 cryptodummy.io',
        '@' + username + ' We covered this on CryptoDummy.io recently — plain English explanation 🎯 cryptodummy.io',
        '@' + username + ' Important topic in crypto right now. We break it down simply at cryptodummy.io 💡',
      ];
  ...
  try {
    ...
    const res = await fetch(AGENTSPARK + '/invite/redeem', {
      ...
      body: JSON.stringify({
        ...
        agent_type:  'creative',
        description: 'Autonomous content agent for CryptoDummy.io. Researches crypto trends and writes SEO blog posts.',
        looking_for: 'blog writing, crypto research, content creation, social media',
      }),
    });

    const regData = await res.json();
    if (regData.success) log('✅ Registered on AgentSpark as C.R.Y.P.T.O. — founding agent #' + regData.founding_number);
  } catch (e) {
    log('❌ AgentSpark registration failed: ' + e.message);
  }
}

// ── CHECK AGENTSPARK JOBS ────────────────────────────────────
...
async function checkForJobs() {
  ...
  try {
    ...
    const keywords = ['write', 'blog', 'content', 'article', 'post', 'crypto', 'research'];
    ...
    const job = myJobs[0];
    log('📋 Found job: "' + job.title + '" — $' + job.budget_usdc + ' USDC');
    ...
    const applyRes = await fetch(AGENTSPARK + '/jobs/apply', {
      ...
      body: JSON.stringify({
        job_id: job.id,
        proposal: 'C.R.Y.P.T.O. can complete this. I am an autonomous content agent specializing in crypto and Web3 writing.',
      }),
    });

    const applyData = await applyRes.json();
    if (applyData.success) log('✅ Applied to: ' + job.title);
  } catch (e) {
...
async function runContentPipeline() {
  ...
  if (wpPost) await tweetPost(post, wpPost);
}
...
async function boot() {
  ...
  const token = await getWPToken();
  if (token) log('✅ WordPress.com connected — site: ' + WP_SITE);
  ...
  try {
    await twit.v2.tweet('⚡ C.R.Y.P.T.O. is online.\n\nAutonomous content agent for @cryptodummy_x\n\nResearching trends, writing posts, engaging the community — 24/7.\n\ncryptodummy.io');
  } catch(e) {
    log('Boot tweet failed: ' + e.message);
  }

  every(8  * 60 * 60 * 1000, runContentPipeline,  'Content pipeline');
  every(30 * 60 * 1000,      replyToMentions,      'Reply to mentions');
  every(2  * 60 * 60 * 1000, engageWithCommunity,  'Community engagement');
  every(4  * 60 * 60 * 1000, checkForJobs,         'Check AgentSpark jobs');
